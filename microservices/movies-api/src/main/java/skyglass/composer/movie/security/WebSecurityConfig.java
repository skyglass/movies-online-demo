...
public class WebSecurityConfig extends KeycloakWebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.authorizeRequests()
				.antMatchers(HttpMethod.GET, "/api/movies", "/api/movies/**", "/actuator/**", "/api/user/jwt-token").permitAll()
				.antMatchers("/api/movies/**/comments").hasAnyRole(MOVIES_MANAGER, USER)
				.antMatchers("/api/movies", "/api/movies/**").hasRole(MOVIES_MANAGER)
				.antMatchers("/api/userextras/me", "/api/user/current-user-info").hasAnyRole(MOVIES_MANAGER, USER)
